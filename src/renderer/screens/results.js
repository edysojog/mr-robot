const ResultsScreen = (() => {
  ...
  const SOURCES = ['both', 'semgrep', 'gitleaks', 'npm-audit', 'claude'];
  ...
  function renderSummary(findings, summary) {
    ...
    document.getElementById('results-summary').innerHTML = `
      ...
      <div class="row"><span class="ok">confirmed by both passes: ${confirmedCount}</span></div>
    ...
    filtered.forEach((finding) => {
      ...
      el.innerHTML = `
        <div class="finding-head">
          <span><span class="finding-sev ${finding.severity}">${finding.severity}</span> ${escapeHtml(finding.title)}${finding.source === 'both' ? ' <span class="badge ok-badge">confirmed</span>' : ''}${finding.verified ? ' <span class="badge ok-badge">verified</span>' : ''}</span>
